...
v4CategoryLabel=function(kind,item){const parent=V4_CATEGORIES[kind].find(p=>p.id===item.primary);return kind==='floor'&&!item.secondary?(parent?.name||'未设置产品线'):`${parent?.name||'未设置产品线'} · ${parent?.children.find(p=>p.id===item.secondary)?.name||'未设置空间'}`;};
