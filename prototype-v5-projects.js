...
const V5_PROJECTS=[{id:'project-one',name:'花香壹号',sort:1,status:'启用'},{id:'project-two',name:'花香二号',sort:2,status:'启用'},{id:'project-chun',name:'春和',sort:3,status:'启用'}];
...
function v5ActiveProject(id){return v5Project(id)?.status==='启用';}
...
v4CategoryLabel=function(kind,item){const parent=V4_CATEGORIES[kind].find(p=>p.id===item.primary);return kind==='floor'?(parent?.name||'未设置业态'):`${parent?.name||'未设置业态'} · ${parent?.children.find(p=>p.id===item.secondary)?.name||'未设置空间'}`;};
