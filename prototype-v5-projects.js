...
V3_EFFECT_PRESETS[0]?.points.push(
...
function v5OpenCaseMaterial(){
 const point=V3_EFFECT_PRESETS[S.v4CaseIndex]?.points?.find(p=>p.id===S.v4PointSelected);
...
function v5AdminFilter(kind){S.v5AdminFilters||={};return S.v5AdminFilters[kind]||= {project:'',business:'',space:''};}
function v5SetAdminFilter(kind,field,value){const f=v5AdminFilter(kind);f[field]=value;if(field==='business')f.space='';render();}
function v5AdminMatches(item,kind){const f=v5AdminFilter(kind);return (!f.project||(f.project==='__none__'?!item.projectId:item.projectId===f.project))&&(!f.business||item.primary===f.business)&&(!f.space||item.secondary===f.space);}
function v5ResetAdminFilters(kind){S.v5AdminFilters[kind]={project:'',business:'',space:''};S.adminQuery='';S.adminStatus='all';render();}
...
function v5ProjectDialog(){const f=S.v5ProjectForm;return `<div class="v3-mask"><section class="v3-admin-dialog v5-project-dialog" role="dialog" aria-modal="true" aria-label="项目编辑"><header class="v3-dialog-head"><h2>${f.id?'编辑':'新增'}项目</h2><button class="v3-icon-btn" aria-label="关闭项目编辑" onclick="S.v5ProjectForm=null;render()">×</button></header><div class="v3-dialog-body"><div class="v3-admin-form"><label><span class="v5-field-caption">项目名称 <em class="v5-required">*</em></span><input aria-required="true" aria-label="项目名称" maxlength="30" placeholder="请输入项目名称，最多30个字" value="${v3Esc(f.name)}" oninput="S.v5ProjectForm.name=this.value"></label><label><span class="v5-field-caption">业态 <em class="v5-required">*</em></span><select aria-label="项目业态" required aria-required="true" aria-describedby="v5-project-error" onchange="S.v5ProjectForm.business=this.value"><option value="">请选择业态</option>${V4_CATEGORIES.floor.map(p=>`<option value="${p.id.slice(2)}" ${f.business===p.id.slice(2)?'selected':''}>${v3Esc(p.name)}</option>`).join('')}</select></label><label><span class="v5-field-caption">排序 <em class="v5-required">*</em></span><input aria-required="true" aria-label="项目排序" type="number" min="1" step="1" value="${f.sort}" oninput="S.v5ProjectForm.sort=this.value"></label></div><p id="v5-project-error" class="v5-project-error" role="alert">${v3Esc(S.v5ProjectError)}</p></div><footer class="v3-dialog-foot"><button class="v3-btn" onclick="S.v5ProjectForm=null;render()">取消</button><button class="v3-btn primary" onclick="v5SaveProject()">保存</button></footer></section></div>`;}
function v5SaveProject(){const f=S.v5ProjectForm,name=f.name.trim(),sort=Number(f.sort);if([...name].length>30){S.v5ProjectError='项目名称不能超过30个字';render();return;}if(!name||!Number.isInteger(sort)||sort<1){S.v5ProjectError='请输入项目名称和大于 0 的整数排序';render();return;}if(!V4_CATEGORIES.floor.some(p=>p.id==='f-'+f.business)){S.v5ProjectError='请选择项目业态';render();document.querySelector('[aria-label="项目业态"]')?.focus();return;}if(V5_PROJECTS.some(p=>p.id!==f.id&&p.name===name)){S.v5ProjectError='项目名称不能重复';render();return;}if(f.id)Object.assign(v5Project(f.id),{name,sort,business:f.business});else V5_PROJECTS.push({id:'project-'+Date.now(),name,sort,business:f.business,status:'启用'});for(const kind of ['floor','effect']){for(const item of V3_ADMIN_ITEMS[kind])if(item.projectId===f.id)v5ApplyProject(kind,item);v4SyncLibrary(kind);}S.v5ProjectForm=null;render();}
